 ...
 */
JNIEXPORT void JNICALL Java_io_github_sammers21_jliburing_IOUring_read0(JNIEnv *env, jobject zthis, jstring fname, jint size, jint offset)
{
    jclass zclass = env->GetObjectClass(zthis);
    jfieldID ringField = env->GetFieldID(zclass, "ringAddress", "J");
    const char *cfname = env->GetStringUTFChars(fname, 0);
    int fd = open(cfname, O_RDONLY);
}
 ...
 */
JNIEXPORT jobject JNICALL Java_Lib_io_1uring_1read_1efd(JNIEnv *env, jobject zthis, jstring fname)
{
}
 ...
 * Signature: (II)J
 */
JNIEXPORT jlong JNICALL Java_io_github_sammers21_jliburing_IOUring_ring_1init(JNIEnv *env, jobject zhis, jint depth, jint flags)
{
    struct io_uring rring;
    int res = io_uring_queue_init(depth, &rring, flags);
    return (long)&rring;
}
